...
def load_data(ticker1, ticker2, start, end):
    df1 = yf.download(ticker1, start=start, end=end, auto_adjust=True)
    df2 = yf.download(ticker2, start=start, end=end, auto_adjust=True)
    return df1, df2
...
def update_plot(data, indicators, sync_axis=None):
    ...
    for indicator in indicators:
        if indicator == "30 Day SMA":
            ...
            par = np.polyfit(
                range(len(df.index.values)), df["Close"].values, 1, full=True
            )
            slope = par[0][0]
            intercept = par[0][1]
...
def on_button_click(main_stock, comparison_stock, start, end, indicators):
    ...
    p1 = update_plot(source1, indicators)
    p2 = update_plot(source2, indicators, sync_axis=p1.x_range)
    curdoc().clear()
